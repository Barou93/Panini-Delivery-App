...
module.exports.getCarts = async (req, res) => {
    ...
    try {
        ...
        const allCartItems = await OrderItem.findAndCountAll({
            limit: size,
            offset: page * size,
        ...
        return res.send({
            content: allCartItems.rows,
            totalPages: Math.ceil(allCartItems.count / Number.parseInt(size))
        })
...
module.exports.addQuantity = async (req, res) => {
    ...
    try {
        ...
        if (addQty) {
            ...
            addQty.save()
                .then(() => {
                    return res.status(200).json(`Votre commande à  ${addQty.quantity} quantités`);
                })
                .catch((err) => {
            ...
            return res.status(404).json("Le produit séléctionner n'est plus disponible")
        }
...
module.exports.removeQuantity = async (req, res) => {
    ...
    try {
        ...
        if (removeQty) {
            ...
            removeQty.save()
                .then(() => {
                    return res.status(200).json(`Votre commande a  ${removeQty.quantity} quantités`)
                })
                .catch((err) => {
                    return res.status(400).json(err)
                })
...
module.exports.deleteAllItems = async (req, res) => {
    try {
        ...
        if (cart) {
            ...
            const deleteAllItems = await OrderItem.destroy({ where: {}, truncate: true, cascade: false });
